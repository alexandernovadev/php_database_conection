<?php 
  ...
  // Para abrir sesiones hay que iniciallizar el metodo para esto
  ...
  if (!empty($_POST['emaillogin']) && !empty($_POST['passlogin'])) {
    $records = $conexion->prepare('SELECT id, email, pass FROM users WHERE email = :email');
    ...
    if (count($results) > 0 && password_verify($_POST['passlogin'], $results['pass'])) {
      $_SESSION['user_id'] = $results['id'];
      header("Location: /phpazo");
      $message = 'Sirveeeee';
    } else {
